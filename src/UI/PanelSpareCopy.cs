...
{
    /// <summary>
    /// The question whether to keep a spare copy of bindrune.keys outside the profile, for
    /// profiles of Thunderstore Mod Manager and r2modman, whose Update existing profile replaces
    /// the profile folder. It is a box of its own over the panel, asked once the profile keeps
    /// something, and again from the Spare copy button at the top. See SpareCopy.
    /// </summary>
    public static partial class BindrunePanel
    {
        ...
        private static readonly Color Chosen = new Color(1f, 0.75f, 0.38f);
        private static readonly Color Faint = new Color(1f, 1f, 1f, 0.5f);

        private static GameObject _modal;
        private static GameObject _spareButton;

        /// <summary>Set when bindrune.keys was saved, so the next frame looks whether the question is due.</summary>
        private static bool _askSpare;

        private static void OnSpareFollowed() => _askSpare = true;
        ...
        /// </summary>
        /// <param name="sayingNo">
        /// The second step after No thanks: going without a spare copy is going without your own
        /// keys after such an update, so it is said plainly once more before it counts.
        /// </param>
        private static void ShowSpareCopyModal(bool sayingNo)
        {
            ...
            if (sayingNo)
            {
                ...
                Wrapped("Without one, Update existing profile takes your own keys, muted clashes and hints in this profile " +
                        "with it, and Bindrune has nothing to bring back. Installing or updating a modpack into the profile is " +
                        "still covered, since that leaves the profile folder in place.", box.transform, ModalInner, 15, Color.white);
                if (wanted == true)
                    Wrapped("The spare copy kept now is removed.", box.transform, ModalInner, 15, Chosen);
            ...
            Wrapped("Update existing profile in Thunderstore Mod Manager and r2modman replaces this whole profile folder, " +
                    "and your own keys go with it. Bindrune can keep a spare copy of them outside the profile, and bring " +
                    "it back the next time the game starts after such an update.", box.transform, ModalInner, 15, Color.white);
            Wrapped("The copy holds only bindrune.keys, with your keys, muted clashes and hints. It goes in the mod " +
                    "manager's folder for the game, beside its profiles:", box.transform, ModalInner, 15, Color.white);
            Wrapped(SpareCopy.Folder ?? "", box.transform, ModalInner, 13, Faint);
            if (wanted != null)
                Wrapped(wanted.Value ? "A spare copy is kept now. No thanks removes it." : "No spare copy is kept now.",
                    box.transform, ModalInner, 15, Chosen);
        ...
        private static void AnswerSpareCopy(bool keep)
        {
            ...
            if (problem != null)
            {
                Note("The spare copy: " + problem + ".");
                return;
            }

            Sfx.Play(Sfx.KeySet);
            Note(keep
                ? "A spare copy of your keys is kept beside the profiles, brought up to date whenever they change."
        ...
        /// <summary>
        /// The Spare copy button at the left end of the buttons at the top, shown only for a
        /// profile that can have one, saying whether it has. Nothing sits to its left, so hidden
        /// it leaves no gap.
        /// </summary>
        private static void BuildSpareButton(float right, float row)
        {
            _spareButton = Button("Spare copy", _root.transform, SpareButtonWidth, 32f, () => ShowSpareCopyModal(false));
            AnchorRight(_spareButton, right, row);
            UpdateSpareButton();
        }
