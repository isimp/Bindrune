...
| Package | One zip per store with the manifest, icon, README, CHANGELOG, LICENSE, the DLL and `docs/for-mod-authors.md`. The Thunderstore zip's manifest also lists `denikson-BepInExPack_Valheim-5.4.2350`. |
...
`packaging/manifest.json` declares `ValheimModding-Jotunn-2.30.1`, which has the same name on both stores, so mod managers install Jotunn along with Bindrune. The plugin itself only declares Jotunn as a soft dependency, but the panel and the hints are built with Jotunn's GUI and nothing is shown without it.
