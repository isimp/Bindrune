...
1. Set the new version in `Bindrune.csproj` (`<Version>`), `src/Plugin.cs` (the `BepInPlugin` attribute) and `packaging/manifest.json` (`version_number`). The workflow checks that all three agree.
...
| Package | `Bindrune-X.Y.Z.zip` with the manifest, icon, README, CHANGELOG, LICENSE, the DLL and `docs/for-mod-authors.md`. |
...
## The Jotunn dependency

`packaging/manifest.json` declares `ValheimModding-Jotunn-2.30.0`, Jotunn's package name on Hexium, so mod managers install it along with Bindrune. The plugin itself only declares Jotunn as a soft dependency, but the panel and the hints are built with Jotunn's GUI and nothing is shown without it. BepInExPack is assumed by the site and is not listed.
...
```bash
HEXIUM_TOKEN=... python3 tools/publish-hexium.py dist/Bindrune-0.1.0.zip --team isimp --categories "Quality of Life,User Interface,Config,Open Source,Valheim 1.0"
python3 tools/publish-hexium.py dist/Bindrune-0.1.0.zip --check-only   # validate only
```

Valid category slugs are listed at `https://hexium.gg/api/experimental/community/valheim/category/`.

## After a game or Jotunn update
...
The script reads the installed Valheim, BepInEx and Jotunn, replaces every method body with `throw null` and writes the result to `lib/`. The stubs can be compiled against but not run. If you add a reference to `Bindrune.csproj`, add it to the list in the script as well.
